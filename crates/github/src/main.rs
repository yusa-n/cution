...
impl GithubTrendingFetcher {
    async fn new(supabase_url: &str, supabase_key: &str, supabase_bucket: &str) -> Result<Self, FetchError> {
        ...
        Ok(Self { http_client, supabase_client, languages })
    }
    ...
    async fn fetch_trending_for_language(&self, language: &str) -> Result<Vec<Repository>, FetchError> {
        ...
        let article_selector = scraper::Selector::parse("article.Box-row")
            .map_err(|e| FetchError::HtmlParse(format!("Failed to parse article selector: {}", e)))?;
        let name_selector = scraper::Selector::parse("h2.h3 a")
            .map_err(|e| FetchError::HtmlParse(format!("Failed to parse name selector: {}", e)))?;
        let desc_selector = scraper::Selector::parse("p.col-9")
            .map_err(|e| FetchError::HtmlParse(format!("Failed to parse description selector: {}", e)))?;
        ...
        for article in document.select(&article_selector) {
            ...
            let description = article.select(&desc_selector).next()
                .map(|p| p.text().collect::<String>().trim().to_string());

            let stars = article.select(&stars_selector).next()
        ...
        info!("Found {} repositories for language '{}'", repositories.len(), if language.is_empty() { "overall" } else { language });
        Ok(repositories)
    }
    ...
    fn stylize_repository_info(&self, repository: &Repository) -> String {
        MARKDOWN_FORMAT
            ...
            .replace("{description}", repository.description.as_deref().unwrap_or("No description provided."))
    }
    ...
    async fn run(&self) -> Result<()> {
        ...
        for language in &self.languages {
            match self.fetch_trending_for_language(language).await {
                ...
                Err(e) => {
                    warn!("Failed to fetch trending for language '{}': {}", language, e);
                }
            }
        ...
        if processed_languages > 0 && !all_markdowns.is_empty() {
            ...
            info!("Uploading {} trending repositories to Supabase Storage at {}", all_markdowns.len(), file_path);
            self.supabase_client
                .upload_file(&file_path, file_content, "text/markdown")
                .await
                .map_err(FetchError::SupabaseUpload)?;
            info!("Successfully uploaded trending repositories to {}", file_path);
        } else {
...
impl SupabaseStorageClient {
    ...
    async fn upload_file(&self, path: &str, content: String, content_type: &str) -> Result<(), String> {
        let url = format!(
            "{}/object/{}/{}",
            self.base_url,
            self.bucket_name,
